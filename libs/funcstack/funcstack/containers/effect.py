...
from funcstack.typing._vars import Args, Other, Out
from funcstack.utils.coroutines import run_sync

class Effect(Generic[Out], ABC):
    def map(
        self,
        func: Callable[[Out, Args.kwargs], Other],
        **kwargs
    ) -> 'Effect[Other]':
    ...
    def flat_map(
        self,
        func: Callable[[Out, Args.kwargs], 'Effect[Other]'],
        **kwargs
    ) -> 'Effect[Other]':
...
class Effects:
    ...
    @final
    class Map(Generic[Out, Other, Args], Effect[Out]):
        def __init__(
            self,
            effect: Effect[Other],
            func: Callable[[Other, Args.kwargs], Out],
            **kwargs
        ):
            self.effect = effect
            self.func = func
    ...
    @final
    class FlatMap(Generic[Out, Other, Args], Effect[Out]):
        def __init__(
            self,
            effect: Effect[Other],
            func: Callable[[Other, Args.kwargs], Effect[Out]],
            **kwargs
        ):
            self.effect = effect
            self.func = func
